import { snakeCase, reduce, merge } from "lodash";
...
const reducer = (services: any): any => {
  return reduce(
    services,
    (carry, [service, port, url]) => {
 ...
 */
const getFullServiceUrl = (key: string) => {
 ...
 */
const getBaseServiceUrl = (key: string, port: number) => {
  if (!process.env[key]) {
    return null;
  }
 ...
 */
const getInternalServiceUrl = (prefixService: string) => {
  return (
 ...
 */
const getInternalServiceUrlWithPort = (keyHost: string, keyPort: string) => {
