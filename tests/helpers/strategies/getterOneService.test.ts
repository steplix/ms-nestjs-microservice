...
import { GetterOneServiceStrategy } from '../../../helpers/strategies';
...
describe('GetterOneServiceStrategy', () => {
    //
    // variables
    //
    let strategy: GetterOneServiceStrategy;
    let model: any;
    ...
    beforeEach(async () => {
        // Create new strategy
        strategy = new GetterOneServiceStrategy();

        // Create default and abstract sequelize model
        model = {
            findAll: jest.fn().mockImplementation(() => Promise.resolve(modelMock))
        };
    });
    ...
    describe('find', () => {
        ...
        it('should return an objects when invoke findAll method from model with empty parameters', async () => {
            ...
            expect(await strategy.find(model, options)).toEqual(modelMock);
