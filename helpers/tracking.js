...
// constants
//
const debug = constants_1.trues.includes(String(process.env.TRACKING_DEBUG).toLowerCase());
const enabled = !constants_1.falses.includes(String(process.env.TRACKING_ENABLED).toLowerCase());
...
class Tracking {
     ...
     *   });
     */
    async dispatchEvent(uri, data) {
        try {
            const trackingService = enabled ? services_1.Service.get("tracking") : mockTrackingService;
            const res = await trackingService.post({ uri, data });
            if (debug) {
                logger_1.logger.debug(`Track event [${uri.split("/").pop().toUpperCase()}] with args [${JSON.stringify(data.args)}]`);
            }
            return res;
        }
        catch (e) {
            if (debug) {
                logger_1.logger.warn(`Error on tracking event [${uri.split("/").pop().toUpperCase()}] with args [${JSON.stringify(data.args)}]`, e);
            }
        }
    }
}
